...
public class UI_ItemInventory : UI_Popup, IConnectable<ItemInventory>
{
    ...
    private void DropItemSlot(UI_ItemSlot itemSlot, UI_ItemSlot droppedItemSlot, PointerEventData eventData)
    {
        ...
        {
            var splitPopup = UIManager.Show<UI_ItemSplit>();
            splitPopup.SetEvent(() =>
            {
                _itemInventoryRef.SplitItem(droppedItemSlot.Index, itemSlot.Index, splitPopup.Quantity);
            },
            $"[{droppedItemSlot.ItemRef.Data.ItemName}] {GuideSettings.Instance.ItemSpliteText}", 1, otherStackable.Quantity);
        }
        else
        {
